...
const STORAGE_KEY = "icaad_regbar_dismissed_v1"
...
export default function RegistrationBar() {
  ...
  const dismiss = () => {
    ...
    setVisible(false)
    // Notify Navbar so it can slide up
    window.dispatchEvent(new Event("regbar:dismiss"))
  }
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-5 flex-1 justify-center text-center sm:text-left">
        <span className="text-sm font-semibold text-white leading-snug">
          <span className="text-brand-gold">Call for Papers is Open</span>
          ...
          <span className="text-white/70 text-xs sm:text-sm hidden sm:inline">
            Submit your paper for ICAAD 2026 via Microsoft CMT
          </span>
        </span>
        <a
          href={CMT_URL}
      ...
      <button
        ...
        className="shrink-0 w-7 h-7 flex items-center justify-center text-white/50 hover:text-white hover:bg-white/10 transition-colors rounded-sm"
      >
        <X size={15} />
      </button>
    </div>
