...
import { ArrowRight, MapPin, FlaskConical, HeartPulse, Sprout, Scale, TrendingUp, Users, BookOpen, Sparkles, Palette, Compass, Globe } from "lucide-react"
    ...
    <>
          ...
          <div className="grid lg:grid-cols-12 gap-14 lg:gap-10 items-start">
            ...
            <MotionDiv variants={staggerContainer} initial="hidden" animate="visible" className="lg:col-span-7">
              <MotionDiv variants={fadeUpVariant}>
                <p className="font-mono text-xs sm:text-sm uppercase tracking-[0.25em] text-brand-gold mb-6 flex flex-wrap items-center gap-x-4 gap-y-2">
                  ...
                  <span className="flex items-center gap-2 text-brand-blackLight">
                    <MapPin size={14} className="text-brand-gold" /> {t.common.location}
                  </span>
                  <span className="text-brand-gold/50" aria-hidden>✦</span>
                  <span className="flex items-center gap-1.5 text-brand-gold font-semibold">
                    <Globe size={14} /> {t.common.hybrid}
                  </span>
                </p>
              </MotionDiv>
              ...
              <MotionDiv variants={fadeUpVariant}>
                <p className="font-mono text-xs uppercase tracking-[0.25em] text-brand-blackLight mb-8">
                  {t.homeNew.organizedBy} {getLocalizedText(conferenceData, lang, "organizer")}
                </p>
              </MotionDiv>
              ...
              <MotionDiv variants={fadeUpVariant} className="mb-10">
                <Countdown targetDate={conferenceData.startDateISO} lang={lang} />
              </MotionDiv>
              ...
              <MotionDiv variants={fadeUpVariant} className="flex flex-col sm:flex-row gap-4">
                ...
                <Link
                  href={`/${lang}/program`}
                  prefetch={false}
                  className="inline-flex items-center justify-center h-12 px-8 text-sm font-bold bg-brand-black text-brand-gold hover:bg-brand-deep transition-all duration-200 w-full sm:w-auto"
                >
                  {t.common.viewProgram}
                </Link>
                <Link href={`/${lang}/cfp`} prefetch={false}>
            ...
            >
              <div className="relative bg-white border border-brand-border shadow-soft overflow-hidden group">
                ...
                {/* Artwork container */}
                <div className="relative overflow-hidden border-b border-brand-border w-full group">
                  <Image
                    ...
                    priority
                  />
                  {/* Tooltip Overlay describing Logo Philosophy */}
                  <div className="absolute inset-x-0 bottom-0 bg-brand-black/90 backdrop-blur-md border-t border-brand-gold/30 p-5 md:p-6 translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-out flex flex-col justify-end max-h-[85%] overflow-y-auto scrollbar-thin scrollbar-thumb-brand-gold">
                    <h4 className="font-display font-semibold text-brand-gold text-sm tracking-wide mb-2 uppercase">
                      {t.homeNew.logoPhilosophyTitle}
                    </h4>
                    <p className="text-xs text-brand-gray font-light leading-relaxed text-justify font-sans">
                      {t.homeNew.logoPhilosophyDesc}
                    </p>
                  </div>
                </div>
                ...
                <dl className="divide-y divide-brand-border bg-white">
                  {[
                    { dt: t.homeNew.dateLabel, dd: getLocalizedText(conferenceData, lang, "date") || t.common.date },
                    { dt: t.homeNew.format, dd: t.common.hybrid },
                    { dt: t.homeNew.venueLabel, dd: t.homeNew.venueText },
                    { dt: t.homeNew.sessions, dd: t.homeNew.sessionsText },
                    { dt: t.homeNew.abstractBook, dd: t.homeNew.abstractBookText },
                    { dt: t.homeNew.fee, dd: t.homeNew.feeText },
                  ].map((row) => (
        ...
        <div className="container mx-auto px-6 max-w-7xl grid grid-cols-2 md:grid-cols-4 divide-x divide-brand-gold/20 relative z-10">
          {conferenceData.stats.map((stat) => (
            <div key={stat.label} className="py-7 px-4 text-center">
              ...
              <p className="font-mono text-[11px] uppercase tracking-[0.2em] text-brand-gray mt-2">
                {stat.label === "Conference Days" && t.homeNew.statsDays}
                {stat.label === "Disciplinary Tracks" && t.home.tracksTitle}
                {stat.label === "Subtopic Domains" && t.homeNew.statsDomains}
                {stat.label === "Paper Sessions" && t.homeNew.statsSessions}
              </p>
            </div>
          ))}
        </div>
      ...
      <Section id="overview" className="bg-brand-bg">
        <div className="grid md:grid-cols-2 gap-16 items-center">
          <MotionDiv variants={fadeUpVariant}>
            <SectionHeading
              eyebrow={t.nav.about}
              title={t.homeNew.overviewTitle}
            />
            <p className="text-lg text-brand-blackLight leading-relaxed mb-6 font-light">
              {conferenceData.organizer} {t.homeNew.overviewP1} ({conferenceData.shortName}): {t.homeNew.overviewP1b}
            </p>
            <p className="text-lg text-brand-blackLight leading-relaxed mb-8 font-light">
              {t.homeNew.overviewP2}
            </p>
            <Link href={`/${lang}/about`} prefetch={false} className="inline-flex items-center gap-2 text-brand-gold font-semibold hover:gap-4 transition-all text-sm uppercase tracking-wide">
              {t.homeNew.overviewLink} <ArrowRight size={16} />
            </Link>
          </MotionDiv>
      ...
      <Section id="tracks" className="bg-brand-surface border-y border-brand-border">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8">
          <SectionHeading
            eyebrow={t.nav.conference}
            title={t.home.tracksTitle}
            description={t.home.tracksDesc}
          />
                  ...
                  <ul className="space-y-1.5 text-xs text-brand-blackLight font-light">
                    ...
                    {track.topics.length > 3 && (
                      <li className="pt-1">
                        ...
                        >
                          +{track.topics.length - 3} {t.common.readMore} →
                        </Link>
                      </li>
                    )}
                  </ul>
      ...
      <Section id="speakers" className="bg-brand-bg">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8">
          <SectionHeading
            eyebrow={t.homeNew.speakersEyebrow}
            title={t.homeNew.speakersTitle}
            description={t.homeNew.speakersDesc}
          />
          <MotionDiv variants={fadeUpVariant} className="shrink-0">
            <Link href={`/${lang}/keynotes`} prefetch={false}>
              <Button variant="outline">{t.homeNew.speakersAll}</Button>
            </Link>
          </MotionDiv>
        </div>
          ...
          <div className="grid md:grid-cols-3 gap-10">
            {allSpeakers.map((speaker) => (
              <MotionDiv key={speaker.id} variants={fadeUpVariant}>
                <Link href={`/${lang}/keynotes`} prefetch={false} className="group block">
                  ...
                  <p className="font-mono text-[11px] uppercase tracking-[0.18em] text-brand-gold mb-2">{getLocalizedText(speaker, lang, "role")}</p>
                  <p className="text-brand-blackLight text-sm font-light">{getLocalizedText(speaker, lang, "affiliation")}</p>
                </Link>
              </MotionDiv>
            ))}
          </div>
          ...
          {importantDates.map((date, i) => (
            ...
            >
              <span className={cn("font-mono text-sm tabular-nums", date.passed ? "text-brand-blackLight line-through" : "text-brand-gold")}>
                {getLocalizedText(date, lang, "date")}
              </span>
              <span className="text-lg font-display font-medium text-brand-black">
                {getLocalizedText(date, lang, "event")}
                {date.passed && <span className="ml-3 font-mono font-normal text-[10px] uppercase tracking-[0.2em] text-brand-blackLight no-underline">{t.homeNew.datesClosed}</span>}
              </span>
            </MotionDiv>
          ))}
        </ol>
      ...
      <Section id="collaborators" className="bg-brand-bg">
        <SectionHeading
          eyebrow={t.homeNew.collabEyebrow}
          title={t.homeNew.collabTitle}
          ...
          >
            ...
            <div>
              <div className="flex items-center justify-between gap-4 mb-6 pb-6 border-b border-brand-border">
                <div className="relative h-16 w-full overflow-hidden flex items-center">
                  <Image
                    ...
                    alt={`${getLocalizedText(conferenceData.hostInstitution, lang, "name")} logo`}
                    width={240}
                    height={70}
              ...
              <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-brand-black bg-brand-gold px-3 py-1 font-bold inline-block mb-4">
                {getLocalizedText(conferenceData.hostInstitution, lang, "role")}
              </span>
              <h3 className="text-2xl font-display font-semibold text-brand-black mb-1">
                {getLocalizedText(conferenceData.hostInstitution, lang, "name")}
              </h3>
              <p className="font-mono text-xs text-brand-blackLight/80 mb-4">{getLocalizedText(conferenceData.hostInstitution, lang, "fullName")}</p>
              <p className="text-brand-blackLight font-light text-sm leading-relaxed mb-6">
                {getLocalizedText(conferenceData.hostInstitution, lang, "description")}
              </p>
            </div>
            ...
            >
              ...
              <div>
                ...
                <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-brand-black bg-brand-goldLight border border-brand-gold/40 px-3 py-1 font-semibold inline-block mb-4">
                  {getLocalizedText(partner, lang, "role")}
                </span>
                <h3 className="text-2xl font-display font-semibold text-brand-black mb-1 group-hover:text-brand-goldDark transition-colors">
                  {getLocalizedText(partner, lang, "name")}
                </h3>
                <p className="font-mono text-xs text-brand-blackLight/80 mb-4">{getLocalizedText(partner, lang, "fullName")}</p>
                <p className="text-brand-blackLight font-light text-sm leading-relaxed mb-6">
                  {getLocalizedText(partner, lang, "description")}
                </p>
              </div>

              <div className="pt-4 border-t border-brand-border flex items-center justify-between text-xs font-mono">
                <span className="text-brand-blackLight">{t.homeNew.partnerLabel}</span>
                <a
        ...
        <div className="container mx-auto px-6 md:px-12 xl:px-24 max-w-7xl relative z-10">
          <MotionDiv variants={staggerContainer} className="max-w-2xl">
            ...
            <div className="flex flex-col sm:flex-row gap-4 mt-10">
              ...
              <Link href={`/${lang}/venue`} prefetch={false}>
